...
/*   Updated: 2022/02/02 10:16:20 by ryosukearim      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_specifier(const char *fmt, va_list *ap)
{
	...
	{
		i += put_string("0x");
		i += base_hexa(va_arg(*ap, unsigned long long), BASE16_S);
	}
	else if (*fmt == 'd' || *fmt == 'i')
		i += base_deci((long long)va_arg(*ap, int));
	else if (*fmt == 'x')
		i += base_hexa((unsigned long long)va_arg(*ap, unsigned int), BASE16_S);
	else if (*fmt == 'X')
		i += base_hexa((unsigned long long)va_arg(*ap, unsigned int), BASE16_L);
	else if (*fmt == 'u')
		i += base_deci((long long)va_arg(*ap, unsigned int));
	else if (*fmt == '%')
		i += put_char('%');
	return (i);
}
